...
Path=/usr/local/bin/power-architect/
Exec=/usr/local/bin/power-architect/start.sh
Icon=/usr/local/bin/power-architect/PowerArchitectPlus1024.exe
Categories=IDE;Development
Keywords=IDE;JAVA
MimeType=application/java
```
...
```bash
sudo tee /usr/share/applications/ZAP.desktop<<EOF
[Desktop Entry]
Version=1.0
Type=Application
Name=ZAP
Path=/usr/local/etc/ZAP_2.11.1
Icon=/usr/local/etc/ZAP_2.11.1/zap.ico
Exec=/usr/local/etc/ZAP_2.11.1/zap.sh
Comment=ZAP
Categories=Development;JAVA;
Terminal=false
EOF
